...
class GameMemoryReader(ABC):
    ...
    def read_warps(self) -> list:
        """Return warp entries for the current map. Override in game-specific reader."""
        return []

    def read_signs(self) -> list:
        """Return sign coordinates for the current map. Override in game-specific reader."""
        return []

    def read_sprites(self) -> list:
        """Return sprite objects (NPCs, items) on the current map.

        Each sprite dict contains:
          y, x       — absolute map coordinates
          picture_id — sprite picture ID (identifies the NPC/item type)
          movement   — movement status byte
          extra_y    — Y position from extra data (2×2 tile grid steps)
          extra_x    — X position from extra data (2×2 tile grid steps)
          type       — \"npc\" | \"item\" | \"unknown\" (best-effort classification)

        Override in game-specific reader.
        """
        return []
