...
class Emulator(ABC):
    ...
    @abstractmethod
    def load(self, rom_path: str, cgb: bool = False, cgb_palette: Optional[list] = None) -> None:
    ...
    def reset(self) -> None:
        """Soft-reset: close and reload the current ROM."""
        rom = self.rom_path
        cgb = getattr(self, '_cgb', False)
        self.close()
        if rom:
            self.load(rom, cgb=cgb)
...
class PyBoyEmulator(Emulator):
    ...
    def load(self, rom_path: str, cgb: bool = False, cgb_palette: Optional[list] = None) -> None:
        ...
        kwargs = {"window": "null", "cgb": cgb, "sound_emulated": False}
        if cgb_palette is not None:
            # Convert flat 12-color list to PyBoy's [[bg],[obj0],[obj1]] format
        ...
        self._pyboy = PyBoy(rom_path, **kwargs)
        # Suppress PyBoy's C-level sound buffer overrun spam
        # (sound_emulated=False disables the Python API but the C layer
        # still fills the audio buffer and logs CRITICAL on every frame)
        import logging
        logging.getLogger("pyboy.core.sound").setLevel(logging.CRITICAL + 1)
        logging.getLogger("pyboy.core.sound").propagate = False
        self._cgb = cgb
        self.rom_path = rom_path
        self.frame_count = 0

    # reset() inherited from base Emulator class
...
def create_emulator(rom_path: str, cgb: bool = False, cgb_palette: Optional[list] = None) -> Emulator:
    ...
    cgb : bool
        If True, force Game Boy Color mode (adds color to .gb ROMs).
    cgb_palette : list, optional
        CGB color palette as 12 hex colors [bg0..bg3, obj0_0..obj0_3, obj1_0..obj1_3].
        Each color is 0xRRGGBB. Converted to PyBoy's [[bg],[obj0],[obj1]] format.
    ...
    emu = cls()
    emu.load(rom_path, cgb=cgb, cgb_palette=cgb_palette)
    return emu
